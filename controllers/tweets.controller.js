...
exports.tweetCreate = async (req, res, next) => {
    try {
        ...
        res.status(400).render('tweets/tweet-form', {errors, isAuthenticated: req.isAuthenticated(), user: req.user});
    }
}
...
exports.tweetDelete = async (req, res, next) => {
    try {
        ...
        const tweets = await getCurrentUserTweetsWithFollowing(req.user);
        res.render('tweets/tweet-list', {tweets, currentUser: req.user, user: req.user, editable: true});
    } catch (e) {
        next(e);
    }
}
...
exports.tweetUpdate = async  (req, res, next)=>{
    ...
    try{
        ...
        const errors = Object.keys(e.errors).map(key => e.errors[key].message);
        const tweet = await  getTweet(tweetId);
        res.status(400).render('tweets/tweet-form', {errors, tweet,  isAuthenticated: req.isAuthenticated(), user: req.user, currentUser: req.user});
    }
}
